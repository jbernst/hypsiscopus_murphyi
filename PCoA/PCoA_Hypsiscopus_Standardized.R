...
Hypsiscopus <- read.csv("Hypsiscopus_MASTER_Morph-Data_PCOA_Adults_SVLStandardized.csv", header = TRUE, na.strings = c("","NA"))
View(Hypsiscopus)

#Check for standardizing by body length (SVL-head length) or just by SVL
sp.m <- Hypsiscopus %>% filter(Hypsiscopus$taxon == "Hypsiscopus murphi")
View(sp.m)
sp.p <- Hypsiscopus %>% filter(Hypsiscopus$taxon == "Hypsiscopus plumbea")

#First, let's see if our variance is equal using a Fisher's F-test
var.test(sp.m$head_length,sp.p$head_length) #this tells us the variance are equal; F = 0.82125, num df = 60, denom df = 19, p-value = 0.5495

#Therefore we can run a standard two sample t-test
t.test(sp.m$head_length,sp.p$head_length, var.equal = TRUE) #tells us there is a significant difference; t = -1.1656, df = 79, p-value = 0.2473



#Since they are different, we should divide our original dataset by BODY SIZE, since head length is significantly different between the two
#This was already done though and we have it above on line 4
#View(Hyps.stand)
View(Hypsiscopus)
testpcoa <- Hypsiscopus[c(11:13,15,17:42)]
#View(testpcoa)
...
sapply(Hyps.stand, class)  #lets you know which columns are factors (a no-no)
...
pcotest <- pco(Gowerdist,k=2)
View(pcotest)

#Save output as .csv
#Commented out since we already have this and don't want to overwrite it
write.csv(pcotest$points,'pcotestPOINTS_SVL_standAdults.csv')

#Visualize plot

#Don't use the next 4 lines
# hemi2<-read.csv("Hemibungarus_numericc.csv")     
# hemi1<-na.omit(hemi2)
# which(is.na(hemi1))
# hemi<-hemi1[c(6:8,11:16,18:31)]
...
pcoaTESTplot <- read.csv("pcotestPOINTS_SVL_standAdults.csv")       
